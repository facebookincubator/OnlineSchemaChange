    ...
    def validate_args(self):
        if self.args.outfile_dir:
            if not os.path.exists(self.args.outfile_dir):
                raise OSCError("OUTFILE_DIR_NOT_EXIST", {"dir": self.args.outfile_dir})
            if not os.path.isdir(self.args.outfile_dir):
                raise OSCError("OUTFILE_DIR_NOT_DIR", {"dir": self.args.outfile_dir})
