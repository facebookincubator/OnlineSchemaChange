...
class RBRPayload(Payload):
    ...
    def __init__(self, *args, **kwargs):
        super(RBRPayload, self).__init__(*args, **kwargs)
        ...
        self.last_replayed_gtid = 0
        self.last_replayed_binlog = ''
    ...
    def init_binlog_path(self):
        """
        Parse the mysql_version string into a version object
        """
        # Strip out the last part because that's the binlog filename prefix
        self.binlog_base = '/'.join(
                self.mysql_vars['log_bin_basename'].split('/')[:-1])
        print(self.binlog_base)
    ...
    def start_transaction_with_gtid(self):
        """
        Start a transaction.
        """
        self.execute_sql(sql.start_transaction_with_snapshot)
        gtid_set = self.query(sql.get_gtid_set)[0]['gtid_set']
        if not gtid_set:
            raise OSCError("EMPTY_GTID_SET")
        else:
            return gtid_set
    ...
    def start_snapshot(self):
        ...
        # Record the current gtid_executed set as last_replayed_gtid. This is 
        # where we should start to replay binlogs in future stages
        ...
        self.last_replayed_gtid = self.query(
                sql.start_transaction_with_snapshot)
        log.info("Start point: {}".format(self.last_replayed_gtid))
    ...
    def replay_binlog(self, binlog, start_gtid, stop_gtid):
        log.info("Going to replay binlog {} from {} to {}"
                .format(binlog, start_gtid, stop_gtid))
        cmd_segments = ["mysqlbinlog", "--skip-gtid"]
        if start_gtid:
            cmd_segments.append('--exclude-gtids={}'.format(start_gtid))
        if stop_gtid:
            cmd_segments.append('--include-gtids={}'.format(stop_gtid))
        cmd_segments.append(os.path.join(self.binlog_base, binlog))
        binlog_sql_file = os.path.join(self.outfile_dir, 'binlog.sql')
        # Use mysqlbinlog to generate a SQL file on disk
        util.os_run(cmd_segments, binlog_sql_file, self.sudo)
        # Use `source` to execute the SQL file
        self.execute_sql("source {}".format(binlog_sql_file))

    def get_binlog_list(self):
        """
        Get a list of binlog file names
        """
        binlogs = self.query(sql.show_binlogs)
        return [r['Log_name'] for r in binlogs]

    def filter_binlogs_to_replay(self, binlogs):
        """
        Get a list of binlog file names since last replay
        """
        binlogs = self.query(sql.show_binlogs)
        binlogs_to_replay = []
        for idx, binlog in enumerate(binlogs):
            # We may not have replayed all the events in the last binlog from
            # the previous attempt so we should start with this one
            if binlog['Log_name'] >= self.last_replayed_binlog:
                return [r['Log_name'] for r in binlogs[idx:]]
    ...
    def replay_changes(self, single_trx=False, holding_locks=False, 
        ...
        binlogs = self.query(sql.show_binlogs)
        binlogs_to_replay = self.filter_binlogs_to_replay(binlogs)
        log.info("Start to replay from binlog {}".format(binlogs_to_replay[0]))

        replayed_total = 0
        showed_pct = 0
        for binlog_file in binlogs_to_replay:
            ...
            self.replay_binlog(
                    binlog_file, self.last_replayed_gtid, replay_until)
            replayed_total += 1
            ...
            progress_pct = int(replayed_total / len(binlogs_to_replay) * 100)
            if progress_pct > showed_pct:
                log.info(
                    "Replay progress: {}/{} changes"
                    .format(replayed_total + 1, len(binlogs_to_replay)))
                showed_pct += 10
        # Commit for last batch
        if not single_trx:
            self.commit()
        self.last_replayed_gtid = replay_until

        time_spent = time.time() - stage_start_time
        self.stats['time_in_replay'] = \
            self.stats.setdefault('time_in_replay', 0) + time_spent
        log.info("Replayed {} binlog files in {:.2f} Seconds"
                 .format(len(binlogs_to_replay), time_spent))
    ...
    def checksum_full_table(self):
        ...
        for more detail
        """
        current_last_binlog_file = self.get_binlog_list()[-1]
        replay_until = self.query(
                sql.start_transaction_with_snapshot)
        ...
        self.replay_changes(single_trx=True, replay_until=replay_until)
        self.last_replayed_binlog = current_last_binlog_file
    ...
    def checksum(self):
        ...
        if not self.detailed_mismatch_info:
            ...
            self.replay_changes(single_trx=False, replay_until=replay_until)
    ...
    def replay_till_good2go(self):
        ...
        for i in range(self.replay_max_attempt):
            ...
            current_last_binlog_file = self.get_binlog_list()[-1]
            replay_until = self.start_transaction_with_gtid()
            self.commit()
            self.replay_changes(single_trx=False, replay_until=replay_until)
            self.last_replayed_binlog = current_last_binlog_file
    ...
    def swap_tables(self):
        ...
        current_gtid = self.start_transaction_with_gtid()
        stage_start_time = time.time()
